...
setwd("C:/Users/Ramona Bledea/Documents/PROJECT RESULTS/IDH Tumor Project/All Idats")
baseDir <- getwd()
outDir <- getwd()
...
#targets <- read.metharray.sheet(baseDir, "targets_file.csv", recursive=TRUE)
targets_450k <- read.metharray.sheet(baseDir, "targets_file_450k.csv", recursive=TRUE)
targets_850k <- read.metharray.sheet(baseDir, "targets_file_850k.csv", recursive=TRUE)
...
legend("topright", legend=levels(f), col=pal[factor(levels(f))], pch=15,cex = 0.7)
dev.off()
png(file=paste(outDir,"/Heatmap(Top 5000).png", sep=""),width=2050,height=2048,pointsize=50)
heatmap.2(gset.funnorm.beta[row.names(dmp[1:5000,]),],trace = 'none',key.title="Methylation", labRow=FALSE, labCol = targets$Sample_Name, cexCol = 1.2, scale = 'column',col = cell_colors,cexRow = 0.52,key.xlab = "BetaValue",main = "Heatmap (Top 5000)",ColSideColors = pal[factor(targets$Sample_Group)])
legend("topright", legend=levels(f), col=pal[factor(levels(f))], pch=15,cex = 0.7)
dev.off()
png(file=paste(outDir,"/Heatmap(Top 10000).png", sep=""),width=2050,height=2048,pointsize=50)
heatmap.2(gset.funnorm.beta[row.names(dmp[1:10000,]),],trace = 'none',key.title="Methylation", labRow = FALSE, labCol = targets$Sample_Name, cexCol = 0.5, scale = 'column',col = cell_colors,cexRow = 0.52,key.xlab = "BetaValue",main = "Heatmap (Top 10000)",ColSideColors = pal[factor(targets$Sample_Group)])
